...
object clock {
  ...
  //DONE: ADD onTIMEOUT to onTICK LISTENER to get timer to work...
  /** A listener for onTick events coming from the internal clock model.  */
  trait OnTickListener {
    def onTick(): Unit
  ...
  class DefaultClockModel(listener: => OnTickListener) extends ClockModel {
    ...
    // DONE make accurate by keeping track of partial seconds when canceled etc.
